...
def export_to_excel(save_path, tags, x_list, y_list, fit_results_list, atomic_percent):
    """
    全データをExcelファイルに出力する関数 (WPF/C#連携・標準リスト対応版)
    save_path: 保存先のファイルパス (.xlsx)
    tags: タグのリスト (list of str)
    x_list, y_list: 全データのx, yリスト (list of list of float)
    fit_results_list: フィッティング結果の辞書リスト (フィッティング無しは None)
                      想定構造: {'y_bg': [...], 'y_total': [...], 'peaks': [{'name':'...', 'y_data':[...], ...}]}
    atomic_percent: 原子組成比のリスト (list of float)
    """
        ...
        with pd.ExcelWriter(save_path, engine='openpyxl') as writer:
            ...
            for i, tag in enumerate(tags):
                ...
                if fit_results_list[i] is not None:
                    ...
                    # バックグラウンド
                    data['Background'] = y_bg
                    
                    # ★修正箇所: 標準listの '+' は連結になるため、zipを使って要素ごとに加算する
                    # 全体のフィッティングカーブ (Total Fit + Background)
                    data['Total Fit'] = [t + b for t, b in zip(res['y_total'], y_bg)]
                    
                    # 各成分 (Component + Background)
                    for peak in res['peaks']:
                        col_name = f"Comp: {peak['name']}"
                        data[col_name] = [p + b for p, b in zip(peak['y_data'], y_bg)]
